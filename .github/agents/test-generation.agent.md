...
- An HTTP request (`this.request.post`, `this.request.get`, `_client.PostAsJsonAsync`, `_client.GetAsync`, `fetch`)
- A Playwright page interaction (`this.page.goto`, `this.page.getByRole`, `this.page.getByLabel`, `this.page.click`)
- An assertion (`expect(...)`, `.Should()`, `.toBe(...)`)
...
1. **Every step definition must contain real test code** — actual HTTP requests (`this.request.post(...)`, `_client.PostAsJsonAsync(...)`), Playwright interactions (`this.page.goto(...)`, `this.page.getByRole(...).click()`), and assertions (`expect(response.status()).toBe(201)`, `response.StatusCode.Should().Be(...)`). The test body IS the implementation contract.
