...
**Stack:** Next.js (TypeScript, App Router) + .NET 10 (Minimal API)

### Project Structure

```
shells/nextjs-dotnet/
├── src/
...
│       ├── Program.cs                # Entry point with endpoint definitions
│       ├── Api.csproj
│       ├── Dockerfile                # .NET container
│       └── tests/                    # xUnit + Reqnroll test project
│           ├── Api.Tests.csproj
│           ├── Unit/                 # xUnit unit tests
│           ├── Features/             # Reqnroll step definitions (.feature bindings)
...
|---|---|---|
| Unit tests (API) | `cd src/api && dotnet test` | xUnit + Reqnroll, runs all .NET tests |
| Unit tests (API, watch) | `cd src/api && dotnet watch test --project tests` | Re-runs on file changes |
...
| Backend | `cd src/api && dotnet run` | http://localhost:5000 (dev) / 8080 (container) |
...
| Backend | `cd src/api && dotnet build` |
